...
| ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| A | Loss | 在 `student/losses.py` 中实现至少 3 种新 loss | `mse`与你实现的所有loss（例如`l1_loss`） | 初始化、优化器、调度器、模型开关 | 最终 PSNR、曲线平滑性、收敛速度 | 无 |
| B | 初始化策略 | 在 `student/initializers.py` 中实现至少 2 种图像感知初始化 | `random` vs 你的方法 | loss、优化器、调度器、模型开关 | 前 50 步收敛速度、最终 PSNR | 重点看前期收敛 |
